...
const Payment = (props) =>{
    ...
    const [receipt, setReceipt] = useState(1234)
    useEffect(()=>{
  ...
  const paymentStart = () => {
    ...
    const paymentCustomertoServer=(userId,homeMakerId)=>{
        ...
        console.log(receipt)
        axios.post(Base_Url+'/customer/create_order/'+userId+'/'+homeMakerId,{amount: amount, receipt:receipt}).then((response) => {
           
            console.log('response', response)
            //status of razorpay client object (order) created at backend
            if(response.data.status === "created")
            {
                ...
                let options={
                    ...
                    currency:'INR',
                    name:'FOOD FROM HOME (TMS)',
                    description:'Payment',
                    image:'/FFH_Logo1.jpeg',
