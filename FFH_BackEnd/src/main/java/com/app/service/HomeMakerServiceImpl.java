...
//			get all customers of HomeMaker and set HomeMaker for all those customers as null
			List<Customer> customers = homeMakerRepo.getAllCustomersForHomeMaker(id);
			customers.forEach(c -> {
				c.setHomeMaker(null);
			});
			System.out.println(customers);
//			set the list of customers for the given  homemaker to null
//			=> unlink the relationship between homemaker and all it's customers
			homeMakerRepo.getById(id).setCustomers(null);
...
//	to get details of all orders for a homemaker
	@Override
	public List<OrderDTO> getAllOrdersForHomeMaker(int hmId) {

		List<OrderDTO> orders = new ArrayList<>();
		orderRepo.getAllOrdersForHomeMaker(hmId).forEach(o -> {
